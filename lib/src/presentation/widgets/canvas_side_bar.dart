...
class _CanvasSideBarState extends State<CanvasSideBar> {
  ...
  void _sendUndoStroke() {
    // if (widget.allSketches.value.isNotEmpty) {
            ...
            child: ListView(
              ...
              children: [
                      ...
                      child: Column(
                        ...
                        children: [
                          Container(
                            width: 22,
                            height: 2,
                            color: widget.drawingTool.value == DrawingTool.line ? Colors.grey[900] : Colors.grey,
                          ),
                        ],
                      ),
                    ),
                ...
                Wrap(
                  children: [
                    // TextButton(
                    //   onPressed: widget.allSketches.value.isNotEmpty ? () => _sendUndoStroke() : null,
                    //   child: const Text('Undo'),
                    // ),
                    ValueListenableBuilder<List<Stroke>>(
                      valueListenable: widget.undoRedoStack.strokesNotifier,
                      builder: (_, strokesNotifier, __) {
                        return TextButton(
                          onPressed: strokesNotifier.isNotEmpty ? () => _sendUndoStroke() : null,
                          child: const Text('Undo'),
                        );
                      },
                    ),
                    ValueListenableBuilder<bool>(
                      valueListenable: widget.undoRedoStack.canRedo,
                      builder: (_, canRedo, __) {
                        return TextButton(
                          onPressed: canRedo ? () => _sendRedoStroke() : null,
