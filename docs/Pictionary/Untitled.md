hoje tenho um bug meio confuso no meu codigo, testei o seguinte cenario:
- coloquei apenas dois participants em uma sala, logo um sempre desenha e o outro sempre adivinha. so para fins de testes rapidos mesmo.
- respondi(guess) a resposta correta alternando entre os usuarios pois sempre um desenha e o outro acerta a palavra. entao eu respondia a resposta correta pelo participant 1, acabava o turno, e no proximo turno respondia a palavra correta pelo participant 2 e assim alternando nos próximos 6 turnos, sempre respondendo o mais rapido possivel, ou seja, uma media de 1s para cada participant, é como se o turno 1 durasse 1s, o turno 2 1s, o truno 3 1s e assim ate o turno 6. Oq eu notei é q com isso, agora ocorre um bug q, de tempos em tempos os turnos eles alternam de maneira independente e nao aleatoriamente, sempre 6 vezes trocando 6 turnos nos intervalos de 1s. preciso q me ajude a corrigir onde q no meu codigo esta ocorrendo esse bug, onde q ele esta salvando esse timer e executando em paralelo provavelmente essas trocas de turnos.
  codigo hoje:
  package main
